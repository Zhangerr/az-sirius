...
namespace mathparse
{
    ...
    {     
        ...
        //old crappy code
        /*
        static double findPrevNum(string s, int pos)
        {
            string num = "";
        ...
        static double findNextNum(string s, int pos)
        {
            string num = "";
        ...
        static void processChunk(string s)
        {
            double sumsofar = 0;
            bool inited = false;
            for (int i = 0; i < s.Length; i++)
            {
                switch (s[i])
                {
                    case '+':
                        if (!inited)
                        {
                            sumsofar = findNextNum(s, i) + findPrevNum(s, i);
                            inited = true;
                        }
                        else
                        {
                            sumsofar += findNextNum(s, i);
                        }
                        break;
                    case '-':
                        if (!inited)
                        {
                            sumsofar = findPrevNum(s, i) - findNextNum(s, i);
                            inited = true;
                        }
                        else
                        {
